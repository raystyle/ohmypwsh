...
- **下载必须校验官方哈希，不能只回填自算 sha256**：自算 sha 是自引用、无信任锚点；应优先从官方 `SHA256SUMS`（`SumsAsset` + `SumsPattern`，名称可用 `{version}`/`{tag}`）或逐资产 `.sha256`（`AssetShaSuffix`）解析目标资产哈希并校验；无官方校验源的工具（age/git/aria2/7z/fnm/jq/ast-grep/oscdimg/dotnet）只能 WARN + 版本校验兜底
