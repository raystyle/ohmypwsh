...
$expected = 'D:\ohmyenv\codex\bin\codex.exe'
...
if ($cmd.Source -eq $expected -and $version -match '0\.148\.0') {
    Write-Host '[OK] PASS：当前终端解析到原生 codex 0.148.0，可以在此会话继续使用' -ForegroundColor Green
    exit 0
} else {
